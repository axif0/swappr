...
const Navbar = () => {
  ...
  const navItems = (
    <>
      <li>
        <Link to="/" className="text-green" onClick={closeDropdown}>
          Home
        </Link>
      </li>
      <li>
        <Link to="/swaptable" className="text-green" onClick={closeDropdown}>
          Available Deals
        </Link>
      </li>
      <li>
        <Link to={`/swap/${user}`} className="text-green" onClick={closeDropdown}>
          My Deals
        </Link>
      </li>
    </>
  ...
  return (
    <header className={`max-w-screen-2xl container mx-auto fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${isSticky ? 'bg-gray-900 shadow-lg' : ''}`}>
      <div
        className="navbar"
